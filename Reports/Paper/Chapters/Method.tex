...
Class rebalancing is added to assist the network in coping with this existing skewness of the training data set. This is done by generating a histogram of the discrete colors in the dataset, using this information to balance the loss function with respect to each color. The histogram of the dataset can be found in figure \ref{fig:histogram}. This histogram is then fused with a uniform distribution to deal with the fact that some of the colors in the histogram are so underrepresented that they have too big of an influence on the color rebalancing. To keep the color rebalancing within reasonable bounds, its expected value is kept equal to one. 
...
Finally, different feature extraction, reconstruction and color generation architectures are combined in five different neural network model architectures that are tested for their performance. These five architectures are summarized in table \ref{tab:dataset}. All of the network architectures are described below by how and why they differ from the other architectures.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.95\textwidth,trim={0cm 17.97cm 2.5cm 6.3cm},clip]{final_architecture.pdf}
	\caption{Compact}
	\label{fig:compact}
\end{figure}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.95\textwidth,trim={0cm 0.5cm 7cm 26.9cm},clip]{final_architecture.pdf}
	\caption{Legend}
	\label{fig:Legend}
\end{figure}

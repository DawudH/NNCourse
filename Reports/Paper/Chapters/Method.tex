...
Before the bottleneck, the image is reduced to a set of features, containing no information about its location in the image. To be able to colorize, spatial information is important. For reconstruction, various methods are used to retrieve the original image resolution.

First of all, the feature maps can be upscaled using linear upscaling. However, to retrieve the original image not only the features but also the information of where those features are coming from have to be retrieved. This is done by concatenating the information before the bottleneck, with the same resolution, with the upscaled features after the bottleneck. Then a convolutional layer is used to merge these features in feature maps of the upscaled resolution. This process is repeated until the original image resolution is retrieved.

Another technique used is the use of dilated convolutions. %Jopie.

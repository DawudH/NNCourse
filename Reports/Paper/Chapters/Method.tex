...
In order to test the different architecture features as described above, five different neural network model architectures were designed. These architectures are implemented and tested for their performance. A summary of the architectures can be found in table \ref{tab:dataset}. All of the network architectures are described below by how and why they differ from the other architectures. There are a few different options for the architecture that are researched. These are all described above, and a short summary is given here:
\begin{itemize}
...
The compact classifier is the same network as the compact network, with a classification output instead of the two feature map output. 

\subsubsection{Compact classifier dilated}
The compact classifier dilated removes the max-pool layers, but instead uses stride to downscale in the feature extraction layers and dilation in the reconstruction layer. 

\subsubsection{Compact classifier dilated and concat}
This network not only uses dilated convolutions but adds concatenation of the feature extraction network with the reconstruction network to add more spatial information.

\subsubsection{VGG16 compact classifier}
The VGG16 compact classifier is the same network as the compact classifier, but instead uses the weights of the VGG16 network for the feature extraction part of the network, also making these weights non-trainable.

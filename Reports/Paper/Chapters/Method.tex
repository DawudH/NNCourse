...
In order to test the different architecture features as described above, five different neural network model architectures were designed. These architectures are implemented and tested for their performance. A summary of the architectures hyperparameters and properties used in the experiments can be found in table \ref{ap:NNset}. All of the network architectures are described below by how and why they differ from the other architectures. There are a few interesting options for the architecture that are researched. The networks are also illustrated in different figures, for which the legend is found in figure \ref{fig:Legend}.
